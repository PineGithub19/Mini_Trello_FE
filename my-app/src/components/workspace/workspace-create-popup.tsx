...
type WorkspaceCreateFormData = z.infer<typeof workspaceCreateSchema>;
const WorkspaceCreatePopup = ({ trigger }: { trigger: React.ReactNode }) => {
  const { data: defaultImages } = useFetchDefaultBackgroundList();
  const { mutate: createWorkspace } = useCreateWorkspaces();
  ...
  const onSubmit = (data: WorkspaceCreateFormData) => {
    createWorkspace(data, {
      ...
      onError: (error) => {
        toastError(error || "Failed to create workspace");
      },
    });
  };
